...
cl_device_id Mandelbrot::createDevice() {
...
cl_context Mandelbrot::createContext(cl_device_id device) {
    cl_int error = 0;
...
cl_program Mandelbrot::buildProgram(cl_context context, cl_device_id device) {
    cl_int error = 0;
...
std::string Mandelbrot::getProgramText() {
...
cl_kernel Mandelbrot::createKernel(cl_program program) {
    cl_int error = 0;
...
cl_command_queue Mandelbrot::createQueue(cl_context context, cl_device_id device) {
    cl_int error = 0;
...
cl_mem Mandelbrot::createBuffer(cl_context context, size_t size) {
    cl_int error = 0;
...
void Mandelbrot::invokeKernel(cl_kernel kernel, cl_command_queue queue, cl_mem buffer, cl_uint *result,
...
size_t Mandelbrot::align(int x, size_t size) {
    return (x + size - 1) / size * size;
}

void Mandelbrot::save(const cl_uint *pixels, int width, int height) {
...
void Mandelbrot::reportError(cl_int status, const std::string function) {
    if (status == CL_SUCCESS) {
        return;
    }
    std::ostringstream stream;
    stream << status;
    std::string message = "OpenCL error code " + stream.str() + " at Mandelbrot::" + function + "()";
