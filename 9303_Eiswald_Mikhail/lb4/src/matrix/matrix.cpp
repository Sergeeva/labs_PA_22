...
Matrix Matrix::slice(int from_row, int to_row, int from_column, int to_column) const {
    Matrix result(to_row - from_row, to_column - from_column);
    for(int i = from_row; i < to_row; ++i) {
        for(int j = from_column; j < to_column; ++j) {
            result.set(i - from_row, j - from_column, this->get(i, j));
        }
    }
    return result;
}
