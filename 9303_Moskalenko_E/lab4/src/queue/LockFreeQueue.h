    ...
    void produce(T element) {
        ...
        while (true) {
            ...
            if (current_tail->next.compare_exchange_weak(null, newTail)) {
                ...
                tail.compare_exchange_weak(current_tail, current_tail->next);  // сдвигаем хвост
            }
        }
    }
    ...
    std::shared_ptr<T> consume() {
        while (true) {
            ...
            if (current_head == current_tail && next_head) {
                tail.compare_exchange_strong(current_tail, next_head); // хвост не был подвинут
            } else if (next_head) {
                auto result = next_head->data;
                if (head.compare_exchange_strong(current_head, next_head)) { // переставили голову
                    return result;
                }
            }
        }
