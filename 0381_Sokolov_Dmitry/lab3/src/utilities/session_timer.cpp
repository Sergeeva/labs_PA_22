...
long long int SessionTimer::get_duration(int index) {
    return duration_cast<std::chrono::milliseconds>(this->sessions[index].second - this->sessions[index].first).count();
}

long long int SessionTimer::last() {
    return duration_cast<std::chrono::milliseconds>(this->sessions.back().second - this->sessions.back().first).count();
}