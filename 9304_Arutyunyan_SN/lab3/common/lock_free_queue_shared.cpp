...
void threaded_push(lock_free_queue<int>& queue, int i) {
    using namespace std::chrono_literals;
    std::this_thread::sleep_for(3000ms);
    std::cout << "pushing from " << i << std::endl;
    queue.push(1*i);
    queue.push(3*i);
}
...
int main() {
    ...
    std::thread t1(threaded_push, std::ref(queue), 1),
                t2(threaded_push, std::ref(queue), 2),
                t3(threaded_push, std::ref(queue), 3);
